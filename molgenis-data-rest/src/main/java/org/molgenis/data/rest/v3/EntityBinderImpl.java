...
public class EntityBinderImpl implements EntityBinder
{
		...
		createEntityRequest.forEach((attributeName, value) ->
		{
			...
			if (attribute == null)
			{
				throw new UnknownAttributeException(
						String.format("Unknown attribute '%s' of entity type '%s'", attributeName, entityType.getId()));
			}
	...
	{
		return createEntitiesRequest.getEntities()
									.stream()
									.map(createEntityRequest -> toEntity(entityType, createEntityRequest))
									.collect(toList());
	}
	...
	private static Instant convertDateTime(Attribute attr, Object paramValue)
	{
		...
		if (paramValue != null)
		{
			...
			else if (paramValue instanceof String)
			{
				...
				try
				{
					value = MolgenisDateFormat.parseInstant(paramStrValue);
				}
	...
	private static LocalDate convertDate(Attribute attr, Object paramValue)
	{
		...
		if (paramValue != null)
		{
			...
			else if (paramValue instanceof String)
			{
				...
				try
				{
					value = MolgenisDateFormat.parseLocalDate(paramStrValue);
				}
				catch (DateTimeParseException e)
				{
					throw new MolgenisDataException(
	...
	private List<?> convertMref(Attribute attr, Object paramValue)
	{
		...
		if (paramValue != null)
		{
			...
			value = mrefParamValues.stream()
								   .map(mrefParamValue -> toEntityValue(mrefEntityIdAttr, mrefParamValue))
