...
/**
 * Servlet filter that checks the httprequest for a molgenis security token and if valid logs the
 * user in, else returns a 401
 */
public class TokenAuthenticationFilter extends GenericFilterBean {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
    ...
    if (!httpRequest.getRequestURI().endsWith("login")
        && !httpRequest.getRequestURI().endsWith("logout")) {
      ...
      if (StringUtils.isNotBlank(token)) {
        try {
          ...
          if (authentication.isAuthenticated()) {
            ...
            Authentication existingAuthentication = securityContext.getAuthentication();
            try {
              securityContext.setAuthentication(authentication);
              chain.doFilter(request, response);
            } finally {
              securityContext.setAuthentication(existingAuthentication);
            }
          } else {
            chain.doFilter(request, response);
          }
