...
class PostgreSqlQueryGenerator {
                ...
                if (isPersistedInOtherTable(attr)) {
                  if (includeMrefs || (attr.getDataType() == ONE_TO_MANY && attr.isMappedBy())) {
                    if (attr.getDataType() == ONE_TO_MANY && attr.isMappedBy()) {
                      ...
                      // TODO retrieve mref values in separate queries to allow specifying limit and
                      // offset after nested MOLGENIS queries are implemented as sub-queries instead
                      // of query rules
                      String mrefSelect =
  ...
  private static String getSqlDefaulValue(Attribute attribute, String defaultValueAsString) {
    ...
    switch (attributeType) {
      ...
      case XREF:
        ...
        sqlDefaultValue =
            getSqlDefaulValue(
                attribute.getRefEntity().getIdAttribute(), refDefaultValue.getIdValue().toString());
        break;
      case DATE:
      ...
      switch (operator) {
        ...
        case EQUALS:
          ...
          if (r.getValue() == null) {
            ...
            // Postgres does not return the rows with an empty value in a boolean field when queried
            // with for example "... NOT abstract = TRUE"
  ...
  static <E extends Entity> String getSqlSort(EntityType entityType, Query<E> q) {
    ...
    // into a unique order.
    // Otherwise you will get an unpredictable subset of the query's rows. You might be asking for
    // the tenth through twentieth rows,
    // but tenth through twentieth in what ordering? The ordering is unknown, unless you specified
    // ORDER BY.
    Sort sort;
