...
RestClient.prototype.get = function (resourceUri, options) {
    return this._get(resourceUri, options, false);
};

RestClient.prototype.getAsync = function (resourceUri, options, callback) {
    return this._get(resourceUri, options, true, callback);
};

RestClient.prototype._get = function (resourceUri, options, async, callback) {
    var resource = null;
...
RestClient.prototype._ajax = function (config) {
    if (self.token) {
        $.extend(config, {
            headers: {
...
RestClient.prototype._toApiUri = function (resourceUri, options) {
    var qs = "";
    if (resourceUri.indexOf('?') != -1) {
...
RestClient.prototype.getPrimaryKeyFromHref = function (href) {
    return href.substring(href.lastIndexOf('/') + 1);
};

RestClient.prototype.getHref = function (entityName, primaryKey) {
    return '/api/v1/' + entityName + (primaryKey ? '/' + primaryKey : '');
};

RestClient.prototype.remove = function (href, callback) {
    return this._ajax({
        type: 'POST',
        url: href,
        data: '_method=DELETE',
...
RestClient.prototype.update = function (href, entity, callback) {
    return this._ajax({
        type: 'POST',
...
RestClient.prototype.entityExists = function (resourceUri) {
    var result = false;
    this._ajax({
        dataType: 'json',
...
RestClient.prototype.login = function (username, password, callback) {
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: '/api/v1/login',
...
RestClient.prototype.logout = function (callback) {
    this._ajax({
        url: '/api/v1/logout',
        async: true,
        success: function () {
